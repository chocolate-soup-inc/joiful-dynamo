...
const TestEntity4 = new index_1.Entity({
    name: 'TestEntity4',
    attributes: {
...
const TestEntity5 = new index_1.Entity({
    name: 'TestEntity5',
    autoExecute: false,
    attributes: {
...
describe('put', () => {
    ...
    it('formats a batch put response', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    it('fails on extra options', () => {
        expect(() => TestEntity.putParams({ email: 'x', sort: 'y' }, 
        // @ts-expect-error
        { extra: true })).toThrow('Invalid put options: extra');
    });
    it('sets capacity options', () => {
        let { TableName, ReturnConsumedCapacity } = TestEntity.putParams({ email: 'x', sort: 'y' }, { capacity: 'none' });
        expect(TableName).toBe('test-table');
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('sets metrics options', () => {
        let { TableName, ReturnItemCollectionMetrics } = TestEntity.putParams({ email: 'x', sort: 'y' }, { metrics: 'size' });
        expect(TableName).toBe('test-table');
        expect(ReturnItemCollectionMetrics).toBe('SIZE');
    });
    it('sets returnValues options', () => {
        let { TableName, ReturnValues } = TestEntity.putParams({ email: 'x', sort: 'y' }, { returnValues: 'ALL_OLD' });
        expect(TableName).toBe('test-table');
        expect(ReturnValues).toBe('ALL_OLD');
    });
    it('fails on invalid capacity option', () => {
        expect(() => TestEntity.putParams({ email: 'x', sort: 'y' }, { capacity: 'test' })).toThrow(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
    });
    it('fails on invalid metrics option', () => {
        expect(() => TestEntity.putParams({ email: 'x', sort: 'y' }, { metrics: 'test' })).toThrow(`'metrics' must be one of 'NONE' OR 'SIZE'`);
    });
    it('fails on invalid returnValues option', () => {
        expect(() => TestEntity.putParams({ email: 'x', sort: 'y' }, 
        // @ts-expect-error
        { returnValues: 'test' })).toThrow(`'returnValues' must be one of 'NONE', 'ALL_OLD', 'UPDATED_OLD', 'ALL_NEW', or 'UPDATED_NEW'`);
    });
    it('sets conditions', () => {
        let { TableName, ExpressionAttributeNames, ExpressionAttributeValues, ConditionExpression } = TestEntity.putParams({ email: 'x', sort: 'y' }, { conditions: { attr: 'email', gt: 'test' } });
        expect(TableName).toBe('test-table');
    ...
    it('handles extra parameters', () => {
        let { TableName, ReturnConsumedCapacity } = TestEntity.putParams({ email: 'x', sort: 'y' }, {}, { ReturnConsumedCapacity: 'NONE' });
        expect(TableName).toBe('test-table');
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('handles invalid parameter input', () => {
        let { TableName } = TestEntity.putParams({ email: 'x', sort: 'y' }, {}, 
        // @ts-expect-error
        'string');
        expect(TableName).toBe('test-table');
    });
