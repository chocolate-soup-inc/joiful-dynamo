...
describe('get', () => {
    it('gets the key from inputs (sync)', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    it('gets the key from inputs (async)', () => __awaiter(void 0, void 0, void 0, function* () {
        const { TableName, Key } = yield TestEntity.get({ email: 'test-pk', sort: 'test-sk' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'test-pk', sk: 'test-sk' });
    }));
    ...
    it('filters out extra data (async)', () => __awaiter(void 0, void 0, void 0, function* () {
        let { TableName, Key } = yield TestEntity.get({
            email: 'test-pk',
            sort: 'test-sk',
            // @ts-expect-error
            test: 'test'
    ...
    it('coerces key values to correct types (async)', () => __awaiter(void 0, void 0, void 0, function* () {
        // @ts-expect-error 💥 TODO: Support coerce keyword
        let { TableName, Key } = yield TestEntity.get({ email: 1, sort: true });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: '1', sk: 'true' });
    }));
    ...
    it('allows execute and parse options', () => {
        let { TableName, Key } = TestEntity.getParams({ email: 'x', sort: 'y' }, { execute: false, parse: false });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    });
    it('fails on extra options', () => {
        expect(() => TestEntity.getParams({ email: 'x', sort: 'y' }, 
        // @ts-expect-error
        { execute: false, parse: false, extra: true })).toThrow('Invalid get options: extra');
    });
    it('fails on invalid consistent option', () => {
        // @ts-expect-error
        expect(() => TestEntity.getParams({ email: 'x', sort: 'y' }, { consistent: 'true' })).toThrow(`'consistent' requires a boolean`);
    });
    it('fails on invalid capacity option', () => {
        // 💥 TODO: Improve capacity type
        expect(() => TestEntity.getParams({ email: 'x', sort: 'y' }, { capacity: 'test' })).toThrow(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
    });
    it('parses attribute projections', () => {
        let { TableName, Key, ExpressionAttributeNames, ProjectionExpression } = TestEntity.getParams({ email: 'x', sort: 'y' }, { attributes: ['email'] });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    ...
    it('sets consistent and capacity options', () => {
        let { TableName, Key, ConsistentRead, ReturnConsumedCapacity } = TestEntity.getParams({ email: 'x', sort: 'y' }, { consistent: true, capacity: 'none' });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    ...
    it('handles extra parameters', () => {
        let { TableName, Key, ConsistentRead } = TestEntity.getParams({ email: 'x', sort: 'y' }, {}, { ConsistentRead: true });
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
        expect(ConsistentRead).toBe(true);
    });
    it('handles invalid parameter input', () => {
        let { TableName, Key } = TestEntity.getParams({ email: 'x', sort: 'y' }, {}, 
        // @ts-expect-error
        'string');
        expect(TableName).toBe('test-table');
        expect(Key).toEqual({ pk: 'x', sk: 'y' });
    });
    it('formats a batch get response', () => __awaiter(void 0, void 0, void 0, function* () {
