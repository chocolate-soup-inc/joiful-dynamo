...
describe('query', () => {
    it('queries a table with no options', () => __awaiter(void 0, void 0, void 0, function* () {
        let result = yield TestTable.query('test', { execute: false });
    ...
    it('fails on an invalid option', () => {
        ...
        // @ts-expect-error
        { invalidParam: true })).toThrow('Invalid query options: invalidParam');
    });
    it('fails on an invalid partionKey', () => {
        // @ts-expect-error
        expect(() => TestTable.queryParams()).toThrow(`Query requires a string, number or binary 'partitionKey' as its first parameter`);
    });
    it('fails on an invalid index', () => {
        expect(() => TestTable.queryParams('test', { index: 'test' })).toThrow(`'test' is not a valid index name`);
    });
    it('fails on an invalid limit', () => {
        expect(() => TestTable.queryParams('test', 
        // @ts-expect-error
        { limit: 'test' })).toThrow(`'limit' must be a positive integer`);
    });
    it('fails on invalid reverse setting', () => {
        expect(() => TestTable.queryParams('test', 
        // @ts-expect-error
        { reverse: 'test' })).toThrow(`'reverse' requires a boolean`);
    });
    it('fails on invalid consistent setting', () => {
        expect(() => TestTable.queryParams('test', 
        // @ts-expect-error
        { consistent: 'test' })).toThrow(`'consistent' requires a boolean`);
    });
    it('fails on invalid select setting', () => {
        expect(() => TestTable.queryParams('test', { select: 'test' })).toThrow(`'select' must be one of 'ALL_ATTRIBUTES', 'ALL_PROJECTED_ATTRIBUTES', 'SPECIFIC_ATTRIBUTES', OR 'COUNT'`);
    });
    it('fails on invalid capacity setting', () => {
        expect(() => TestTable.queryParams('test', { capacity: 'test' })).toThrow(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
    });
    it('fails on invalid entity', () => {
        expect(() => TestTable.queryParams('test', { entity: 'test' })).toThrow(`'entity' must be a string and a valid table Entity name`);
    });
    it('fails on invalid startKey', () => {
        expect(() => TestTable.queryParams('test', { startKey: 'test' })).toThrow(`'startKey' requires a valid object`);
    });
    ...
    it('fails on multiple conditions (lt)', () => {
        expect(() => TestTable.queryParams('test', { eq: 'val', lt: 'val1' })).toThrow(`You can only supply one sortKey condition per query. Already using 'eq'`);
    });
    it('fails on multiple conditions (lte)', () => {
        expect(() => TestTable.queryParams('test', { eq: 'val', lte: 'val1' })).toThrow(`You can only supply one sortKey condition per query. Already using 'eq'`);
    });
    it('fails on multiple conditions (gt)', () => {
        expect(() => TestTable.queryParams('test', { eq: 'val', gt: 'val1' })).toThrow(`You can only supply one sortKey condition per query. Already using 'eq'`);
    });
    it('fails on multiple conditions (gte)', () => {
        expect(() => TestTable.queryParams('test', { eq: 'val', gte: 'val1' })).toThrow(`You can only supply one sortKey condition per query. Already using 'eq'`);
    });
    it('fails on multiple conditions (beginsWith)', () => {
        expect(() => TestTable.queryParams('test', { eq: 'val', beginsWith: 'val1' })).toThrow(`You can only supply one sortKey condition per query. Already using 'eq'`);
    });
    it('fails on multiple conditions (between)', () => {
        expect(() => TestTable.queryParams('test', { eq: 'val', between: ['val1', 'val2'] })).toThrow(`You can only supply one sortKey condition per query. Already using 'eq'`);
    });
    it('fails on in valid between condition', () => {
        expect(() => TestTable.queryParams('test', 
        // @ts-expect-error
        { between: ['val1'] })).toThrow(`'between' conditions requires an array with two values.`);
    });
});
