...
const TestTable3 = new index_1.Table({
    name: 'test-table3',
    partitionKey: 'pk',
    entityField: false,
    DocumentClient: bootstrap_tests_1.DocumentClient
});
const TestEntity3 = new index_1.Entity({
    name: 'TestEntity3',
    autoExecute: false,
    attributes: {
        email: { type: 'string', partitionKey: true },
...
const TestEntityGSI = new index_1.Entity({
    name: 'TestEntityGSI',
    autoExecute: false,
    attributes: {
        email: { type: 'string', partitionKey: true },
...
describe('update', () => {
    it('creates default update', () => {
        let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity.updateParams({ email: 'test-pk', sort: 'test-sk' });
    ...
    it.skip('creates update that removes fields', () => {
        let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity.updateParams({ email: 'test-pk', sort: 'test-sk', $remove: 'test_string' });
    ...
    it('fails removing an invalid attribute', () => {
        // @ts-expect-error
        expect(() => TestEntity.updateParams({ email: 'x', sort: 'y', $remove: 'missing' })).toThrow(`'missing' is not a valid attribute and cannot be removed`);
    });
    it('fails when trying to remove the paritionKey', () => {
        // @ts-expect-error
        expect(() => TestEntity.updateParams({ email: 'x', sort: 'y', $remove: 'pk' })).toThrow(`'pk' is the partitionKey and cannot be removed`);
    });
    it('fails when trying to remove the sortKey', () => {
        // @ts-expect-error
        expect(() => TestEntity.updateParams({ email: 'x', sort: 'y', $remove: ['sk'] })).toThrow(`'sk' is the sortKey and cannot be removed`);
    });
    it('creates update that just saves a composite field', () => {
        let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity2.updateParams({ email: 'test-pk', test_composite: 'test' });
    ...
    it('uses an alias', () => __awaiter(void 0, void 0, void 0, function* () {
        let { TableName, Key, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } = TestEntity.updateParams({
            email: 'test@test.com',
            sort: 'test-sk',
    ...
    it("fails when missing an 'always' required field", () => {
        // @ts-expect-error
        expect(() => TestEntity3.updateParams({ email: 'test-pk' })).toThrow(`'test2' is a required field`);
    });
    ...
    it('fails when supplying non-array value for SET', () => {
        expect(() => TestEntity2.updateParams({ email: 'test-pk' }, {}, 
        // @ts-expect-error
    ...
    it('adds statements to SET, REMOVE, ADD and DELETE (with names and values) and a ConditionExpression', () => {
        let { UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues, ConditionExpression } = TestEntity2.updateParams({ email: 'test-pk', test: 'test' }, {}, {
            SET: ['#field = :field'],
    ...
    it('fails on extra options', () => {
        expect(() => TestEntity.updateParams({ email: 'x', sort: 'y' }, 
        // @ts-expect-error
        { extra: true })).toThrow('Invalid update options: extra');
    });
    it('sets capacity options', () => {
        let { TableName, ReturnConsumedCapacity } = TestEntity.updateParams({ email: 'x', sort: 'y' }, { capacity: 'none' });
        expect(TableName).toBe('test-table');
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('sets metrics options', () => {
        let { TableName, ReturnItemCollectionMetrics } = TestEntity.updateParams({ email: 'x', sort: 'y' }, { metrics: 'size' });
        expect(TableName).toBe('test-table');
        expect(ReturnItemCollectionMetrics).toBe('SIZE');
    });
    it('sets returnValues options', () => {
        let { TableName, ReturnValues } = TestEntity.updateParams({ email: 'x', sort: 'y' }, { returnValues: 'ALL_OLD' });
        expect(TableName).toBe('test-table');
        expect(ReturnValues).toBe('ALL_OLD');
    });
    it('fails on invalid capacity option', () => {
        expect(() => TestEntity.updateParams({ email: 'x', sort: 'y' }, { capacity: 'test' })).toThrow(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
    });
    it('fails on invalid metrics option', () => {
        expect(() => TestEntity.updateParams({ email: 'x', sort: 'y' }, { metrics: 'test' })).toThrow(`'metrics' must be one of 'NONE' OR 'SIZE'`);
    });
    it('fails on invalid returnValues option', () => {
        expect(() => 
        // @ts-expect-error
        TestEntity.updateParams({ email: 'x', sort: 'y' }, { returnValues: 'test' })).toThrow(`'returnValues' must be one of 'NONE', 'ALL_OLD', 'UPDATED_OLD', 'ALL_NEW', OR 'UPDATED_NEW'`);
    });
    it('sets conditions', () => {
        let { TableName, ExpressionAttributeNames, ExpressionAttributeValues, ConditionExpression } = TestEntity.updateParams({ email: 'x', sort: 'y' }, { conditions: { attr: 'email', gt: 'test' } });
        expect(TableName).toBe('test-table');
        expect(ExpressionAttributeNames).toEqual({
            '#test_string': 'test_string',
            '#_ct': '_ct',
    ...
    it('handles extra parameters', () => {
        let { TableName, ReturnConsumedCapacity } = TestEntity.updateParams({ email: 'x', sort: 'y' }, {}, { ReturnConsumedCapacity: 'NONE' });
        expect(TableName).toBe('test-table');
        expect(ReturnConsumedCapacity).toBe('NONE');
    });
    it('handles invalid parameter input', () => {
        let { TableName } = TestEntity.updateParams({ email: 'x', sort: 'y' }, {}, 
        // @ts-expect-error
        'string');
        expect(TableName).toBe('test-table');
    });
