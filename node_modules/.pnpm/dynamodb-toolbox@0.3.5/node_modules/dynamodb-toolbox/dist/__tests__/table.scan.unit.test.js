...
describe('scan', () => {
    ...
    it('fails on an invalid option', () => {
        ...
        // @ts-expect-error
        { invalidParam: true })).toThrow('Invalid scan options: invalidParam');
    });
    it('fails on an invalid index', () => {
        expect(() => TestTable.scanParams({ index: 'test' })).toThrow(`'test' is not a valid index name`);
    });
    ...
    it('fails on invalid select setting', () => {
        expect(() => TestTable.scanParams({ select: 'test' })).toThrow(`'select' must be one of 'ALL_ATTRIBUTES', 'ALL_PROJECTED_ATTRIBUTES', 'SPECIFIC_ATTRIBUTES', OR 'COUNT'`);
    });
    it('fails on invalid capacity setting', () => {
        expect(() => TestTable.scanParams({ capacity: 'test' })).toThrow(`'capacity' must be one of 'NONE','TOTAL', OR 'INDEXES'`);
    });
    it('fails on invalid entity', () => {
        expect(() => TestTable.scanParams({ entity: 'test' })).toThrow(`'entity' must be a string and a valid table Entity name`);
    });
    it('fails on invalid startKey', () => {
        expect(() => TestTable.scanParams({ startKey: 'test' })).toThrow(`'startKey' requires a valid object`);
    });
    it('fails on invalid segments', () => {
        expect(() => TestTable.scanParams(
        // @ts-expect-error
        { segments: 'test' })).toThrow(`'segments' must be an integer greater than 1`);
    });
    it('fails on invalid segment', () => {
        expect(() => TestTable.scanParams(
        // @ts-expect-error
        { segment: 'test' })).toThrow(`segment' must be an integer greater than or equal to 0 and less than the total number of segments`);
    });
    it('fails if both segments and segment are not included', () => {
        expect(() => TestTable.scanParams({ segments: 10 })).toThrow(`Both 'segments' and 'segment' must be provided`);
    });
});
